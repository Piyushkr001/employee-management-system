...
- **Race Condition Protection**: Updates and deletions are re-authorized contextually *inside* the database transaction with `FOR UPDATE` locks. This ensures that changes (e.g., creating an employee under a newly soft-deleted manager, or HR attempting to demote a newly-promoted Super Admin) are securely validated against the absolute latest database state, avoiding any phantom reads.
